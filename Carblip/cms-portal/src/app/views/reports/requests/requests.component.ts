...
import { formatDropdown } from 'app/shared/helpers/multiselect.helper';

@Component({
  selector: 'app-requests',
  templateUrl: './requests.component.html',
  styleUrls: ['./requests.component.scss', '../reports.component.scss'],
  animations: [
    fadeInRight400ms
  ]
})
export class RequestsComponent implements OnInit {
  ...
  getRequestReports() {
    ...
    this.reportService.getRequestsReport(this.requestFilter).subscribe( res => {
      if(res.statusCode == 200 && res.data.length > 0){
        this.requestsReport = res.data[0].chartdata;
        this.contactOwnersList = formatDropdown(res.data[0].contactowner);
        this.requestsReport = this.chartViewGenerateService.createChartData(this.requestsReport, this.requestFilter.start_date, this.requestFilter.end_date);
        this.total = this.chartViewGenerateService.sum(this.requestsReport);
  ...
  changeViewType(event) {
    if(this.requestsReport.length > 0){
      if(event === 'D') {
        this.single = this.chartViewGenerateService.generateDayView(this.requestsReport);
      } else if(event === 'W') {
        this.single = this.chartViewGenerateService.generateWeekView(this.requestsReport);
      } else {
        this.single = this.chartViewGenerateService.generateMonthView(this.requestsReport);
      } 
