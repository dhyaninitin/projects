...
import * as workflowModels from 'app/shared/models/workflow.model';
...
export class WorkflowsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private readonly sectionName: string = 'filter_workflows_page';
      ...
      .pipe(
        ...
        tap(data => {
          if (!deepEqual(this.filter, data)) {
            if(data.search != ""){
              localStorage.setItem("workflow_module_search_keyword", data.search);
            }else{
              localStorage.removeItem("workflow_module_search_keyword");
            }
  ...
  loadData() {
    var per_page_limit = localStorage.getItem('workflow_module_limit');
    var selected_order_dir = localStorage.getItem('workflow_module_order_dir');
    var selected_order_by = localStorage.getItem('workflow_module_order_by');
    var selected_page_no = localStorage.getItem('workflow_module_page_count');
    var search_keyword = localStorage.getItem('workflow_module_search_keyword');
    // Clone filter values
  ...
  onPaginateChange(event) {
    ...
    localStorage.setItem('workflow_module_limit', event.pageSize);
    localStorage.setItem('workflow_module_page_count', data.page);
