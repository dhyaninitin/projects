...
@Component({
  ...
  styleUrls: ['./delay-action.component.scss', './../../create.component.scss']
})
export class DelayActionComponent implements OnInit {
  ...
  editDelayAction(delay: any) {
    const title = 'Edit Dealy Action';
    const actionDialog = this.dialog.open(ActionModalComponent,  {
      width: '720px',
      disableClose: true,
      data: {title: title, data: delay, isEdit: true},
    });
    actionDialog.afterClosed().subscribe(res => {
      if (res != null) {
        res.seq_id = delay.seq_id;
        let outputobj = {
          value:'edit_action',
          data:res,
          container: this.delay_action,
          action_id: this.id,
          sequence_id: delay.seq_id
        }
        this.output.next(outputobj)
      }
    })
