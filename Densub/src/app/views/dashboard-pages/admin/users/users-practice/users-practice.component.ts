...
import { Filter } from './users-practice.filter';
import { AccessLevelService } from '../../access-level/access-level.service';

@Component({
  selector: 'app-users-practice',
  templateUrl: './users-practice.component.html',
  styleUrls: ['./users-practice.component.scss']
})
export class UsersPracticeComponent implements OnInit {
  ...
  getUsersData() {
    this.spinner.show();
    const condition = {
      userType: this.userTypes.PRACTICE
    };
...
/*   validateForm() {
  ...
  saveUserData() {
    ...
    // this.spinner.show();
    if ( !this.currentUser._id) {
      this.currentUser.userType = this.userTypes.PRACTICE;
