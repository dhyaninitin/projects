...
import { Filter } from './users-admin.filter';
...
export class UsersAdminComponent implements OnInit {
  ...
  // validCheckAlreadyExitRole: boolean = true;
  validCheckAlreadyExitEmail: boolean = true;
  ...
  getUsersData() {
    this.spinner.show();
    const condition = {
      userType: { $nin: [ this.userTypes.STAFF, this.userTypes.PRACTICE, this.userTypes.ADMIN]}
    };
  ...
  saveUserData() {
    ...
    //   return false;
    // }
    const found = this.accessLevelList.filter(
    ...
    if ( !this.currentUser._id) {
      ...
      this.currentUser.profileVerificationStatus = environment.PROFILE_STATUS.VERIFIED;
    }
