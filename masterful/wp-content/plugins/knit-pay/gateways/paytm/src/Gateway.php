...
class Gateway extends Core_Gateway {
	...
	private function get_order_id( Payment $payment ) {
		...
		$payment_description = str_replace( ' ', '_', $payment_description ); // Change space to underscore.
		$payment_description = preg_replace( '/[^a-zA-Z0-9_-]/s', '', $payment_description ); // Remove special characters.
		
		$order_id = $payment->key . '_' . $payment_description;

		$order_id = substr( $order_id, -50 );

		$payment->add_note( 'paytm_order_id: ' . $order_id );
		$payment->set_meta( 'paytm_order_id', $order_id );
