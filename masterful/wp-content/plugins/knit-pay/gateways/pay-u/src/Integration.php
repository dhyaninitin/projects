	...
	public function get_settings_fields() {
		...
		if ( ! defined( 'KNIT_PAY_PAYU' ) ) {
			$fields[] = [
				...
				'html'    => sprintf(
					/* translators: 1: PayU */
					__( 'Knit Pay supports %1$s with a Premium Addon. But you can get this premium addon for free.%2$s', 'knit-pay' ),
					__( 'PayU', 'knit-pay' ),
		...
		$fields[] = [
			...
			'html'    => sprintf(
				/* translators: 1: PayU */
				__( 'Before proceeding, kindly create an account at %1$s if you don\'t have one already.%2$s', 'knit-pay' ),
				__( 'PayU', 'knit-pay' ),
				'<br><a class="button button-primary" target="_blank" href="http://go.thearrangers.xyz/payu?utm_source=knit-pay&utm_medium=ecommerce-module&utm_campaign=module-admin&utm_content=help-signup"
					...
					/* translators: 1: PayU */
				__( '<br><br>For Testing, kindly create an account at %1$s if you don\'t have one already.%2$s', 'knit-pay' ),
				__( '<strong>PayU UAT Dashboard</strong>', 'knit-pay' ),
		...
		if ( ! defined( 'KNIT_PAY_PAYU_BIZ_API' ) ) {
			...
			$fields[] = [
				'section'  => 'general',
				'title'    => __( 'Registered Phone Number', 'knit-pay' ),
				'type'     => 'description',
				'callback' => [ $this, 'field_send_phone_otp' ],
				'tooltip'  => __( 'Phone number registered at PayU.', 'knit-pay' ),
			];

			// Registered Email.
			$fields[] = [
				'section'  => 'general',
				'title'    => __( 'Registered Email', 'knit-pay' ),
				'type'     => 'description',
				'callback' => [ $this, 'field_send_email_otp' ],
				'tooltip'  => __( 'Email address registered at PayU.', 'knit-pay' ),
			];
			...
			$fields[] = [
				'section' => 'general',
				'type'    => 'html',
				'html'    => __( 'Merchant Keys and Salt will be saved automatically on successful OTP verification.', 'pronamic_ideal' ),
			];
		}
		...
		$fields[] = [
			...
			'description' => 'Merchant Salt is available on <a target="_blank" href="https://www.payu.in/business/payment-gateway/integration">Integration Page</a>. You can use anyone from Merchant Salt, Merchant Salt v1, or Merchant Salt v2.',
		];
		...
		if ( ! defined( 'KNIT_PAY_PAYU_BIZ_API' ) ) {
			...
			$fields[] = [
				'section' => 'general',
				'type'    => 'html',
				'html'    => '<script src="' . plugins_url( '', __FILE__ ) . '/js/admin.js"></script>',
			];
		}
