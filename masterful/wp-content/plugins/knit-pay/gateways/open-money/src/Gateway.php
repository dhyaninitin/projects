...
class Gateway extends Core_Gateway {
	...
	public function start( Payment $payment ) {
		...
		if ( ! defined( 'KNIT_PAY_OPEN_MONEY' ) ) {
			$error = sprintf(
				...
				__( 'Knit Pay supports %1$s with a Premium Addon. By signing up Open Money using Knit Pay\'s referral link you can get this premium addon for free. Visit the Knit Pay website (knitpay.org) to know more.', 'knit-pay' ),
		...
		if ( ! isset( $layer_payment_token['id'] ) ) {
			throw new \Exception( reset( reset( $layer_payment_token['error_data'] ) ) );
		}
	...
	private function get_payment_data( Payment $payment ) {
		...
		return [
			...
			'mtx'            => date( 'ymd' ) . '-' . rand( 1, 100 ) . '-' . $payment->get_id(),
		];
	}
