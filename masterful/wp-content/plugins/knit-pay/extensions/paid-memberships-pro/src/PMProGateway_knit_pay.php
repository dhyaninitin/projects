...
class PMProGateway_knit_pay extends PMProGateway {
	...
	function init() {
		...
		if ( $gateway == $this->id ) {
			...
			add_action( 'pmpro_checkout_before_form', [ $this, 'hide_billing_address' ] );
		}
	}
	...
	function pmpro_payment_option_fields( $values, $gateway ) {
		...
		$form .= '>
            ...
            <td>
            	...
            	<p class="description">' . __( 'Payment Method title visible on the payment confirmation page and invoice page.', 'knit-pay' ) . '</p>
            </td>
        </tr>';
		...
		$form .= '	</select>
        		<p class="description">' . __( 'Hide not required billing address fields on the checkout page.', 'knit-pay' ) . '</p>
        	</td>
        </tr>';
	...
	function hide_billing_address() {
		...
		$script  = '<style>';
		$script .= '.pmpro_checkout-field-baddress1,';
		$script .= '.pmpro_checkout-field-baddress2,';
		$script .= '.pmpro_checkout-field-bcity,';
		$script .= '.pmpro_checkout-field-bstate,';
		$script .= '.pmpro_checkout-field-bzipcode,';
		$script .= '.pmpro_checkout-field-bcountry';
		$script .= '{display: none;}';
		$script .= '</style>';
		echo $script;
	}
