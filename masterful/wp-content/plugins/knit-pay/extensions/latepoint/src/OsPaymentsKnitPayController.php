...
if ( ! class_exists( 'OsPaymentsKnitPayController' ) ) :
	...
	class OsPaymentsKnitPayController extends OsController {
		...
		/* Generates payment options for RazorPay payment modal */
		public function get_payment_options() {
			...
			if ( 0 >= $amount && $this->get_return_format() === 'json' ) {
				...
				$this->send_json(
					[
						...
						'message' => __( 'Nothing to pay', 'latepoint-payments-razorpay' ),
						'amount'  => $amount,
					]
				);
				return;
			...
			try {
				...
				$this->send_json(
					[
						'status'              => LATEPOINT_STATUS_SUCCESS,
						'message'             => __( 'Payment Link Created.', 'latepoint-payments-razorpay' ),
