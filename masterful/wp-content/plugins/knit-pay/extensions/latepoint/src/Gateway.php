...
class Gateway {
		...
		if ( OsPaymentsHelper::is_payment_processor_enabled( $this->processor_code ) && 'knit_pay' === $booking->payment_method ) {
			...
			if ( $booking->payment_token ) {
				...
				if ( PaymentStatus::SUCCESS === $payment->get_status() ) {
				  ...
				  $result['status']  = LATEPOINT_STATUS_ERROR;
				  $result['message'] = __( 'Payment ID undefined.', 'latepoint-payments-paypal' );
				  $booking->add_error( 'payment_error', $result['message'] );
			}
		}
		return $result;
	...
	public function get_supported_payment_methods() {
		return [
			$this->processor_code => [
				'name'      => __( 'Knit Pay', 'knit-pay' ),
				'label'     => __( 'Pay Online', 'knit-pay' ),
	...
	public function add_settings_fields( $processor_code ) {
	  ...
	  <h3><?php _e( 'Knit Pay Settings', 'knit-pay' ); ?></h3>
		 <?php echo OsFormHelper::select_field( 'settings[knit_pay_config_id]', __( 'Configuration', 'knit-pay' ), $configuration_list, OsSettingsHelper::get_settings_value( 'knit_pay_config_id', get_option( 'pronamic_pay_config_id' ) ) ); ?>
	   
	   <div class="os-row">
			<div class="os-col-6">
				  <?php echo OsFormHelper::text_field( 'settings[knit_pay_payment_description]', __( 'Payment Description', 'knit-pay' ), OsSettingsHelper::get_settings_value( 'knit_pay_payment_description', '{service_name}' ) ); ?>
			  </div>
			<div class="os-col-6">
				  <?php echo sprintf( __( 'Available tags: %s', 'pronamic_ideal' ), sprintf( '<code>%s</code>', '{service_name}, {agent_name}, {generated_form_id}' ) ); ?>
			  </div>
	  </div>
	  
		<?php echo OsFormHelper::text_field( 'settings[knit_pay_currency_iso_code]', __( 'ISO Currency Code', 'knit-pay' ), OsSettingsHelper::get_settings_value( 'knit_pay_currency_iso_code' ) ); ?>
