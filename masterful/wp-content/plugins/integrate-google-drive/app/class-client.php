...
class Client {
	...
	public function __construct( $account_id = null ) {
		...
		$this->client_id     = apply_filters( 'igd/client_id', '880105439986-fq4s7da3ps11g48bq5dvskouapfqhff5.apps.googleusercontent.com' );
		$this->client_secret = apply_filters( 'igd/client_secret', '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' );
		$this->redirect_uri  = apply_filters( 'igd/redirect_uri', 'https://wpmilitary.com/gdtowp-oAuthCallback.php' );
	...
	public function start_client() {
		...
		$state = admin_url( 'admin.php?page=integrate-google-drive&action=authorization' );
	...
	public function create_access_token() {
		...
		try {
			...
			$about = App::instance()->getService()->about->get( [ 'fields' => 'storageQuota,user' ] );

			//get root_id
			$root_id = App::instance()->getService()->files->get( 'root' )->getId();
			...
			error_log( 'Google Drive to WP Message: ' . sprintf( 'Couldn\'t generate Access Token: %s', $exception->getMessage() ) );
