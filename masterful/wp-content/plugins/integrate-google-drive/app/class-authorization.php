...
class Authorization {
	...
	public function refresh_token( $account ) {
		...
		if ( empty( $refresh_token ) ) {
			error_log( '[Google Drive to WP MESSAGE]: No Refresh Token found during the renewing of the current token. We will stop the authorization completely.' );
	...
	public function revoke_token() {
		error_log( 'Google Drive to WP - Message: ' . 'Lost authorization' );

		try {
			$this->get_client()->revokeToken();
		} catch ( \Exception $exception ) {
			error_log( 'Google Drive to WP - Error: ' . $exception->getMessage() );
		}
